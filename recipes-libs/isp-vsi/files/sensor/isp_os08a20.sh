...
ISP_SENSOR_I2C=$(echo "$sensor_mode" | cut -d':' -f3)
ISP_SENSOR_MODE=$(echo "$sensor_mode" | cut -d':' -f4)
ISP_SENSOR_CALI=$(echo "$sensor_mode" | cut -d':' -f5)
ISP_SENSOR_MANU=$(echo "$sensor_mode" | cut -d':' -f6)
ISP_SENSOR_AUTO=$(echo "$sensor_mode" | cut -d':' -f7)

echo "sensor port[$ISP_SENSOR_PORT]"
echo " - mipi_id     : $ISP_SENSOR_MIPI"
echo " - i2c_bus     : $ISP_SENSOR_I2C"
...
echo $ISP_SENSOR_PORT i2c_bus=$ISP_SENSOR_I2C > $ISP_VSI_RPOC;
