...
'
	...
	printf "\nStarting %s - '%s', log:%d, trace:%d: " "$DAEMON" "$SENSOR_SCRIPT" "$DEBUG_LEVEL" "$TRACE_LEVEL"
...
usage() {
	...
	echo "  -m <command index>      : Select the sensor command index (default: $SELECT_MODE)"
...
while getopts "c:d:t:s:m:" opt; do
	case ${opt} in
	c)
		COMMAND=$OPTARG
		;;
